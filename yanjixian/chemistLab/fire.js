...
THREE.Fire = function (size, color) {
    ...
    firePlane1 = new THREE.Mesh( new THREE.PlaneGeometry(size, size), material);
    firePlane1.visible = true;

    firePlane2 = new THREE.Mesh( new THREE.PlaneGeometry(size, size), material);
    firePlane2.visible = true;
    ...
    if( color ) {
        ...
        uniforms.color.value.x = colorObj.r + 2.0;
        uniforms.color.value.y = colorObj.g + 2.0;
        uniforms.color.value.z = colorObj.b + 2.0;
    }
    
    firePlane.add(firePlane1);
    firePlane.add(firePlane2);
    ...
    candelFire : {
        ...
        fragmentShader : [
            ...
            "void main( void ) {",
                ...
                "float a = mix(-size * dist, noise, noiseStrength);",
                "gl_FragColor = vec4(a + color, a + max(max(color.r, color.g), color.b));",
                "if(a+color == vec3(0.0,0.0,0.0)) gl_FragColor = vec4(1.0,1.0,1.0,0.0);",
                "}"
